...
/**
 * Sas_Shop Sanitization Helper Class
 *
 ...
 */
class Sas_Shop_Sanitization_Helper {
	...
	public function __construct( $plugin_name ) {
		...
		$this->registered_settings = Sas_Shop_Settings_Definition::get_settings();

		add_filter( 'sas_shop_settings_sanitize_text', array( $this, 'sanitize_text_field' ) );
		add_filter( 'sas_shop_settings_sanitize_email', array( $this, 'sanitize_email_field' ) );
		add_filter( 'sas_shop_settings_sanitize_checkbox', array( $this, 'sanitize_checkbox_field' ) );
		add_filter( 'sas_shop_settings_sanitize_url', array( $this, 'sanitize_url_field' ) );
	}
	 ...
	 *
	 * Filters in order:
	 * - sas_shop_settings_sanitize_<tab_slug>
	 * - sas_shop_settings_sanitize_<type>
	 * - sas_shop_settings_sanitize
	 * - sas_shop_settings_on_change_<tab_slug>
	 * - sas_shop_settings_on_change_<field_key>
	 *
	 * @since 	1.0.0
	...
	public function settings_sanitize( $input = array() ) {
		...
		$tab = isset( $referrer['tab'] ) ? $referrer['tab'] : Sas_Shop_Settings_Definition::get_general_tab_slug();

		// Tab filter
		$input = apply_filters( 'sas_shop_settings_sanitize_' . $tab, $input );
	...
	private function apply_type_filter( $input, $tab, $key ) {
		...
		return apply_filters( 'sas_shop_settings_sanitize_' . $type, $input[$key], $key );
	}

	private function apply_general_filter( $input, $key ) {

		return apply_filters( 'sas_shop_settings_sanitize', $input[$key], $key );
	}

	// Key specific on change hook
	private function do_settings_on_key_change_hook( $key, $new_value ) {

		$old_plugin_settings = get_option( 'sas_shop_settings' );
		...
		if ( isset($old_plugin_settings[$key]) && $old_plugin_settings[$key] !== $new_value ) {
			...
			do_action( 'sas_shop_settings_on_change_' . $key, $new_value, $old_plugin_settings[$key] );
	...
	private function do_settings_on_change_hook( $new_values, $tab ) {
		...
		if ( $changed ) {
			...
			do_action( 'sas_shop_settings_on_change_' . $tab, $new_values, $old_plugin_settings );
