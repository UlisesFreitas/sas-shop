 ...
 * @return string
 */
function sas_shop_sanitize_taxonomy_name( $taxonomy ) {
 ...
 * @return string
 */
function sas_shop_get_filename_from_url( $file_url ) {
 ...
 *
 * Usage: sas_shop_get_dimension(55, 'in');
 *
 * @access public
 * @param mixed $dim
 ...
 * @return float
 */
function sas_shop_get_dimension( $dim, $to_unit ) {

	$from_unit 	= strtolower( get_option( 'sas_shop_dimension_unit' ) );
 ...
 *
 * Usage: sas_shop_get_weight(55, 'kg');
 *
 * @access public
 * @param mixed $weight
 ...
 * @return float
 */
function sas_shop_get_weight( $weight, $to_unit ) {

	$from_unit 	= strtolower( get_option('sas_shop_weight_unit') );
	$to_unit	= strtolower( $to_unit );
 ...
 * @return string
 */
function sas_shop_trim_zeros( $price ) {
 ...
 * @return string
 */
function sas_shop_format_decimal( $number, $dp = false, $trim_zeros = false ) {
	// Remove locale from string
	if ( ! is_float( $number ) ) {
		$locale   = localeconv();
		$decimals = array( get_option( 'sas_shop_price_decimal_sep' ), $locale['decimal_point'], $locale['mon_decimal_point'] );
		$number   = sas_shop_clean( str_replace( $decimals, '.', $number ) );
	}
 ...
 * @return string
 */
function sas_shop_float_to_string( $float ) {
	if ( ! is_float( $float ) ) {
		return $float;
	}
 ...
 * @return array
 */
function sas_shop_array_overlay( $a1, $a2 ) {
    foreach( $a1 as $k => $v ) {
        if ( ! array_key_exists( $k, $a2 ) ) {
        	continue;
        }
        if ( is_array( $v ) && is_array( $a2[ $k ] ) ) {
            $a1[ $k ] = sas_shop_array_overlay( $v, $a2[ $k ] );
        } else {
            $a1[ $k ] = $a2[ $k ];
        }
    }
 ...
 * @return int
 */
function sas_shop_let_to_num( $size ) {
    $l 		= substr( $size, -1 );
    $ret 	= substr( $size, 0, -1 );
    switch( strtoupper( $l ) ) {
	    case 'P':
